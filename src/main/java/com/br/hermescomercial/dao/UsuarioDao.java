...
public class UsuarioDao {
    ...
    public void salvar(Usuario usuario) {
        String sql = "INSERT INTO USUARIO (nome, endereco, bairro, cidade, estado, cep, cnpj, cpf,tipoDocumento, email, tipoUsuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement ps = con.getConnection("Postgres").prepareStatement(sql)) {
            ...
            ps.setString(9, usuario.getTipoDocumento());
    ...
    public void update(Usuario usuario) {
        String sql = "UPDATE USUARIO SET endereco=?, bairro=?, cidade=?, estado=?, cep=?, cnpj=?, cpf=?,tipoDocumento=?, email=?, tipoUsuario=? WHERE nome=?";
        try (PreparedStatement ps = con.getConnection("Postgres").prepareStatement(sql)) {
            ...
            ps.setString(8, usuario.getTipoDocumento());
            ps.setString(9, usuario.getEmail());
            ps.setString(10, usuario.getTipousuario());
            ps.setString(11, usuario.getNome());
            ps.executeUpdate();
        } catch (Exception e) {
