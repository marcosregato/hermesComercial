 ...
 * Implementação de telas correspondentes para cada item do submenu
 */
public class PDVMenuLateralComplete {
    ...
    public PDVMenuLateralComplete(String usuario, String nome, JFrame mainFrame) {
    ...
    private JButton createSubMenuButton(String text, String module) {
        try {
            ...
            // Ação do submenu - IMPLEMENTAÇÃO DE TELAS CORRESPONDENTES
            button.addActionListener(e -> {
                try {
                    ...
                    abrirTelaCorrespondente(text, module);
    ...
    /**
     * Abre a tela correspondente ao item do submenu clicado
     */
    private void abrirTelaCorrespondente(String item, String module) {
        try {
            SystemLogger.ui("Abrindo tela correspondente: " + item);
            
            // Criar frame para a tela correspondente
            JFrame telaFrame = new JFrame(item + " - Hermes Comercial PDV");
            telaFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            telaFrame.setSize(900, 600);
            telaFrame.setLocationRelativeTo(mainFrame);
            ...
            JPanel conteudo = criarConteudoTela(item, module);
            painelPrincipal.add(conteudo, BorderLayout.CENTER);
            
            // Status bar
            JPanel statusBar = criarStatusBar(item);
            painelPrincipal.add(statusBar, BorderLayout.SOUTH);
            
            telaFrame.add(painelPrincipal);
            telaFrame.setVisible(true);
            
            // Log de sucesso
            SystemLogger.ui("Tela " + item + " aberta com sucesso para usuário: " + usuarioAtual);
            
        } catch (Exception e) {
            SystemLogger.error("Erro ao abrir tela correspondente: " + item, e);
            JOptionPane.showMessageDialog(menuPanel, 
    ...
    private JPanel criarHeaderTela(String titulo) {
        ...
        header.add(usuarioLabel, BorderLayout.EAST);
        
        return header;
    }
    
    /**
     * Cria o conteúdo específico da tela
     */
    private JPanel criarConteudoTela(String item, String module) {
        JPanel conteudo = new JPanel(new BorderLayout());
        conteudo.setBackground(Color.WHITE);
        conteudo.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));
        
        // Mensagem de boas-vindas
        JLabel mensagemLabel = new JLabel("🎉 Bem-vindo ao módulo " + item, JLabel.CENTER);
        mensagemLabel.setFont(new Font("Segoe UI", Font.BOLD, 24));
        mensagemLabel.setForeground(new Color(52, 73, 94));
        mensagemLabel.setBorder(BorderFactory.createEmptyBorder(50, 50, 30, 50));
        ...
        conteudo.add(mensagemLabel, BorderLayout.NORTH);
     ...
     */
    private JPanel criarStatusBar(String item) {
